...
public class SECTR_AudioSystem : MonoBehaviour 
{
		...
		public void Update(float deltaTime, bool volumeOnly)
		{
			...
			if(source && (source.isPlaying || Paused || volumeOnly) && !Muted)
			{
				...
				if(HDR)
				{
						...
						{						
							if(listenerDistance > minDistance)
							{
								...
								case SECTR_AudioCue.FalloffTypes.Linear:
									falloff = 1 - Mathf.Clamp01((listenerDistance - minDistance) / (maxDistance - minDistance));
									break;
					...
					attenuatedLoudness += 20f * Mathf.Log10(Mathf.Max(userVolume * fadeVolume * paramVolume * falloff, 0.001f));
...
#else
							lowpass.lowpassResonanceQ = Mathf.Lerp(1, system.OcclusionResonanceQ, effectiveOcclusion);
#endif
						}
					}
				}
			...
			if(source && (source.isPlaying || Paused) && !Local && system.BlendNearbySounds)
			{
				...
				if(listenerDistance <= system.NearBlendRange.x)
				{
					spatialBlend = 0f;
				}
				else if(listenerDistance <= system.NearBlendRange.y)
			   	{
					spatialBlend = Mathf.Clamp01(listenerDistance - system.NearBlendRange.x) / (system.NearBlendRange.y - system.NearBlendRange.x);
				}
				else
				{
					spatialBlend = 1f;
			...
			if(source && numParams > 0)
			{
				...
				while(iter.MoveNext())
				{
					...
					Component component = source.GetComponent(paramData.componentType);
					if(component)
					{
	...
	public static SECTR_AudioCueInstance Play(SECTR_AudioCue audioCue, Transform parent, Vector3 localPosition, bool loop)
	{
		...
		else if(activeInstances.Count >= system.MaxInstances || instancePool.Count == 0)
		{
	...
	void OnEnable()
	{
		...
		else if(system == null)
		{
			...
			if(!HUDLineMaterial)
			{
				HUDLineMaterial = (Material)(AssetDatabase.LoadAssetAtPath("Assets/SECTR/Code/Audio/Assets/AudioHUD_Graph.mat", typeof(Material)));
			}
	...
	private static void _UpdateBusPitchVolume(SECTR_AudioBus bus, float effectiveVolume, float effectivePitch)
	{
		if(bus)
		{
			...
			if(soloBuses != null && soloBuses.Count > 0 && !soloBuses.Contains(bus) && bus != system.MasterBus)
			{
				effectiveVolume = 0;
			}
#endif

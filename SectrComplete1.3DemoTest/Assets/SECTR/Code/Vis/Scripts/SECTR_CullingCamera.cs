...
public class SECTR_CullingCamera : MonoBehaviour 
{
	...
	/// Accessor that allows setting an override/proxy camera that
	/// will be used for culling instead of the camera on this object.
	...
	{	
		...
		bool cullingSimpleCulling = SimpleCulling;
		if(cullingProxy)
		{
			SECTR_CullingCamera proxy = cullingProxy.GetComponent<SECTR_CullingCamera>();
			if(proxy)
			{
				cullingSimpleCulling = proxy.SimpleCulling;
			}
		}
		...
		#endif
		{
			UnityEngine.Profiling.Profiler.BeginSample("LODs");
		...
		if(enabled && cullingCamera.enabled && numInitialSectors > 0)
		{
			...
			UnityEngine.Profiling.Profiler.BeginSample("Graph Walk");
			while(nodeStack.Count > 0)
			{
				VisibilityNode currentNode = nodeStack.Pop();

				UnityEngine.Profiling.Profiler.BeginSample("Culling");
				if(currentNode.frustumPlanes != null)
				{
	...
	#endif
					{
						...
						if(numWorkers > 0)
						{
							...
							{
								cullingWorkQueue.Enqueue(new ThreadCullData(sector, this, cameraPos, cullingPlanes, occluderFrustums, baseMask, shadowDistance, cullingSimpleCulling));
...
#endif
						{
							_FrustumCullSector(sector, cameraPos, cullingPlanes, occluderFrustums, baseMask, shadowDistance, cullingSimpleCulling, ref visibleRenderers, ref visibleLights, ref visibleTerrains, ref shadowLights);
						}
					}
					UnityEngine.Profiling.Profiler.EndSample();
					...
					UnityEngine.Profiling.Profiler.BeginSample("Clipping");
					int numNodePortals = cullingSimpleCulling ? 0 : currentNode.sector.Portals.Count;
			...
			if(numWorkers > 0)
			{
				UnityEngine.Profiling.Profiler.BeginSample("Cull Join");
			...
			UnityEngine.Profiling.Profiler.BeginSample("Shadows");
			   ...
			   )
			{
				...
				UnityEngine.Profiling.Profiler.BeginSample("Shadow Setup");
				...
				UnityEngine.Profiling.Profiler.BeginSample("Shadow Cull");
				...
				if(numWorkers > 0)
				{
					UnityEngine.Profiling.Profiler.BeginSample("Shadow Join");
					while(remainingThreadWork > 0)
					{
						while(cullingWorkQueue.Count > 0)
						{
	...
	private void _FrustumCullSectorThread(ThreadCullData cullData)
	{
		...
		Dictionary<int, int> localVisibleRenderers = null;
		Dictionary<int, int> localVisibleLights = null;
		Dictionary<int, int> localVisibleTerrains = null;
		...
		{
			localVisibleRenderers = threadVisibleListPool.Count > 0 ? threadVisibleListPool.Pop() : new Dictionary<int, int>(32);
			localVisibleLights = threadVisibleListPool.Count > 0 ? threadVisibleListPool.Pop() : new Dictionary<int, int>(32);
			localVisibleTerrains = threadVisibleListPool.Count > 0 ? threadVisibleListPool.Pop() : new Dictionary<int, int>(32);
		}
		...
		Dictionary<int, int>.Enumerator visibleEnum;
		lock(visibleRenderers)
		{
			visibleEnum = localVisibleRenderers.GetEnumerator();
			while(visibleEnum.MoveNext())
			{
				int renderer = visibleEnum.Current.Key;
				visibleRenderers[renderer] = renderer;
			}
		}
		lock(visibleLights)
		{
			visibleEnum = localVisibleLights.GetEnumerator();
			while(visibleEnum.MoveNext())
			{
				int light = visibleEnum.Current.Key;
				visibleLights[light] = light;
			}
		}
		lock(visibleTerrains)
		{
			visibleEnum = localVisibleTerrains.GetEnumerator();
			while(visibleEnum.MoveNext())
			{
				int terrain = visibleEnum.Current.Key;
				visibleTerrains[terrain] = terrain;
			}
		}
		lock(shadowLights)
		{
		...
		if(memberLightVisible)
		{
			...
			for(int lightIndex = 0; lightIndex < numLights; ++lightIndex)
			{
				   ...
				   !visibleLights.ContainsKey(child.lightHash) )
				{
					...
					{
						visibleLights.Add(child.lightHash, child.lightHash);
				   ...
				   !visibleRenderers.ContainsKey(shadowCasterChild.renderHash))
				{
					for(int shadowLightIndex = 0; shadowLightIndex < numShadowLights; ++shadowLightIndex)
					{
						...
						if(((shadowLightChild.shadowCullingMask & 1 << shadowCasterChild.layer) != 0))
						{
							...
							{
								visibleRenderers.Add(shadowCasterChild.renderHash, shadowCasterChild.renderHash);
								break;
							}
						}
					}
				   ...
				   !visibleTerrains.ContainsKey(shadowCasterChild.terrainHash))
				{
					for(int shadowLightIndex = 0; shadowLightIndex < numShadowLights; ++shadowLightIndex)
					{
						...
						if(((shadowLightChild.shadowCullingMask & 1 << shadowCasterChild.layer) != 0))
						{
							...
							{
								visibleTerrains.Add(shadowCasterChild.terrainHash, shadowCasterChild.terrainHash);
								break;
							}
						}
					}
	...
	private void _ApplyCulling()
	{
		...
		for(int memberIndex = 0; memberIndex < numMembers; ++memberIndex)
		{
			...
			SECTR_Member member = SECTR_Member.All[memberIndex];
			int numRenderers = member.Renderers.Count;
			int numVisibleRenderers = numRenderers;
			for(int rendererIndex = 0; rendererIndex < numRenderers; ++rendererIndex)
			{
				SECTR_Member.Child child = member.Renderers[rendererIndex];
				Renderer childRenderer = child.renderer;
				if(childRenderer && !visibleRenderers.ContainsKey(child.renderHash) && childRenderer.enabled)
				{
			...
			int numLights = member.Lights.Count;
			int numVisibleLights = numLights;
			for(int lightIndex = 0; lightIndex < numLights; ++lightIndex)
			{
				SECTR_Member.Child child = member.Lights[lightIndex];
				Light childLight = child.light;
				if(childLight && !visibleLights.ContainsKey(child.lightHash) && childLight.enabled)
				{
			...
			int numTerrains = member.Terrains.Count;
			int numVisibleTerrains = numTerrains;
			for(int terrainIndex = 0; terrainIndex < numTerrains; ++terrainIndex)
			{
				SECTR_Member.Child child = member.Terrains[terrainIndex];
				Terrain childTerrain = child.terrain;
				if(childTerrain && !visibleTerrains.ContainsKey(child.terrainHash) && childTerrain.enabled)
				{
					hiddenTerrains.Add(childTerrain);
					#if OLD_UNITY
					childTerrain.enabled = false;
					#else
					childTerrain.drawHeightmap = false;
					childTerrain.drawTreesAndFoliage = false;
					#endif
					--numVisibleTerrains;
				}
			}
			if (numVisibleRenderers + numVisibleLights + numVisibleTerrains > 0)
			{
				member.LastVisibleFrameNumber = Time.frameCount;
			}
		}
	}
	...
	private void UndoCulling()
	{
		...
		for(int terrainIndex = 0; terrainIndex < numTerrains; ++terrainIndex)
		{
			Terrain terrain = hiddenTerrains[terrainIndex];
			#if OLD_UNITY

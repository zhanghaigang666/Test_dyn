...
public struct HexCoordinates
{
	...
	public static HexCoordinates FromPosition (Vector3 position) {
		float x = position.x / (HexMetrics.innerRadius * 2f);  //奇数行1,3,5
		float y = -x;

        //float offset = position.z / (HexMetrics.outerRadius * 1.5f * 2); //1.5f * 2，偶数行，每2行做偏移
        //float offset =  HexMetrics.outerRadius * 1.5f * 2; //1.5f * 2，偶数行，每2行做偏移
        //float offset =  HexMetrics.outerRadius * 1.5f * 1; //1.5f * 2，偶数行，每2行做偏移
        //float offset =  HexMetrics.outerRadius * 1.5f * 0; //1.5f * 2，偶数行，每2行做偏移
        //float offset =  1/(HexMetrics.outerRadius * 1.5f * 2); //1.5f * 2，偶数行，每2行做偏移
        float offset =  2/(HexMetrics.outerRadius * 1.5f * 2); //1.5f * 2，偶数行，每2行做偏移
        x -= offset;
        y -= offset;
